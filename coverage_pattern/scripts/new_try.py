...
class PatternDrive(New_Visualization):
    def __init__(self):
        ...
        self.tolerance = rospy.get_param("~point_tolerance", 0.2)
    ...
    def odom_callback(self, odom):
        pose_current_x = odom.pose.pose.position.x
        pose_current_y = odom.pose.pose.position.y
        self.current_position = (pose_current_x, pose_current_y)
        #print("Current Position: x={}, y={}".format(pose_current_x, pose_current_y))
    ...
    def create_pattern(self, polygon):
        ...
        self.visualization_square_marker(pattern, self.robot_width, self.robot_length, arrived=False)
        return pattern
    ...
    def drive_pattern(self, pattern):
        ...
        for pos_last, pos_next in pairwise(pattern):
            ...
            self.visualization_arrived(pos_next, self.robot_width, self.robot_length)
            time.sleep(1)

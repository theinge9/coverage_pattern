#! /usr/bin/env python
"""
Zigzag Pattern for polygon grid cells
"""
import numpy as np
...
def create_grid(points, robot_x, robot_y):
    ...
    ## Using inner polygon ##
    #poly_inner = poly.buffer(-robot_x/2)
    #num_row = ceil(inner_x/robot_x)
    #num_column = ceil(inner_y/robot_y)
    ...
    ## Sort the cells in alternating direction based on index ## zigzag ##
    #grid_cells.sort() 
    #grid_cells.sort(reverse=True) 
    #grid_cells.sort(key=lambda cell: cell[1]) ## in ascending order based on values of second element in each cell
    #grid_cells.sort(key=lambda cell: cell[1], reverse=True) ## in decending order
    #grid_cells.sort(key=lambda p: (p[0], -p[1]))
...
if __name__ == "__main__":
    def test(points, width=0.5, length=0.5):

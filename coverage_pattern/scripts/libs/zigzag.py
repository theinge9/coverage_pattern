...
def calc_path1(poly, robot_width, bStartTop=False):
	"""
		...
		if len(intersections_y) < 2:
			...
			for i in range(2): # One to follow the current plow, one to drive to the next plow
				...
				if not pos_cur in ret:
					ret.append(pos_cur)
				path = sort_zigzag(ret)
	return path#ret

def sort_zigzag(waypoints):
...
if __name__ == "__main__":
	...
	def test(points, width=0.1, bStartTop=False):
		fig, ax = plt.subplots()
		poly = Polygon(points)
		path = calc_path3(poly, width, bStartTop)
		print(path)
		#waypoints = sort_zigzag(path)
		#print(waypoints)
		pos_last = None
		for pos_cur in path:
	...
	test([[0.0, 0.3], [1.0, 0.3], [1.0, 1.5], [0.0, 1.5]])
	"""

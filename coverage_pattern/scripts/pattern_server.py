#! /usr/bin/env python3
"""
Pattern Service Server, PublishPoint Tool is used to call
"""
import time
import numpy as np
from math import *
...
class PatternDrive(New_Visualization):
    def __init__(self):
        ...
        rospy.loginfo("Pattern Service is ready...")
    ...
    def pattern_callback(self, request):
        ...
        points = [(p[0].point.x, p[0].point.y) for p in self.clicked_points]
        
        if len(self.clicked_points) > 2:
            # All points must have same frame_id
            if len(set([p[0].header.frame_id for p in self.clicked_points])) != 1:
               raise ValueError()
            
            points_x = [p[0].point.x for p in self.clicked_points]
            points_y = [p[0].point.y for p in self.clicked_points]
        ...
        return patternResponse(result = "Service Running..")
